...
\begin{center}
    ...
    \parbox{0.9\textwidth}{\small
        ...
        lattice size $L=16$, the MLP achieves test MSE $0.62 \pm 0.65$,
        while a single linear layer achieves $0.17 \pm 0.09$
        on the same task; the linear model's lower mean error remains visible,
        but the strength of the difference depends on the statistical test
        because the MLP variance is large (Welch $p=0.055$, Mann-Whitney
        $p=0.026$, Cohen $d=-1.0$).  The linear model's
        advantage is also pronounced in some off-critical same-scale settings.
        ...
        budget; it is methodologically informative but not decisive, and its results are
        highly protocol-sensitive and do not support a stable monotone
        scale-degradation law.  Dedicated sampler diagnostics further show
        strong critical slowing down, with
        $\tau_{\mathrm{int}}(|M|)\approx 37$ sweeps at $(L,\beta)=(16,\beta_c)$,
        so all cross-scale claims are treated as exploratory rather than
        decisive evidence for RG-like behavior.
        A translation-equivariant CNN (with block-spin inductive bias) achieves
        near-zero MSE in almost all tested $(L,\beta)$ combinations, with the
        notable exception of $L=4$, $\beta=0.60$, showing that locality and
        translation structure are sufficient to learn the supervised
        block-spin map, but not by themselves evidence of RG-flow learning.  A
        compressed non-evidentiary turbulence feasibility note is retained in
        the appendix.
        ...
        in this local majority-vote block-spin setting, standard deep
        architectures do not exhibit RG-like advantages on block-spin tasks
        that are well-approximated by locally linear projections.} 
\end{center}
\clearpage
\pagenumbering{roman}
\tableofcontents
...
\begin{enumerate}
    ...
    \item Is this learning statistically meaningful (vs. random baselines)?
        ...
        train the same MLP on identical fine-grained inputs but with
        independently shuffled coarse labels.  If the correct-label model does
        better than this control, the gap measures how much of the task is
        genuine block-spin structure rather than generic memorization.  In the
        present revision that control remains appendix-level because it was
        produced under a separate pilot protocol.
...
\begin{table}[ht]
    ...
    \begin{tabular}{llll}
        ...
        Appendix diagnostics & Pilot / spectral / turbulence / mixing & mixed auxiliary budgets & Non-primary support \\
        \bottomrule
    \end{tabular}}
...
\begin{enumerate}
    ...
    \item[(ii)] \textbf{Error depends strongly on phase and lattice size.}
        ...
        ($0.1178 \pm 0.1695$ for MLP and $0.0106 \pm 0.0099$ for Linear at
        $\beta=0.60$), hardest in the disordered phase for the MLP
        ($0.6006 \pm 0.0287$ at $\beta=0.30$), and most variable at the critical
...
the linear baseline has lower mean test MSE than the MLP at the critical
point.  At $(L,\beta)=(16,\beta_c)$ the MLP reaches
$0.6249 \pm 0.6456$, while Linear reaches
$0.1736 \pm 0.0862$.  The mean ordering favors the linear model, but the
strength of the difference depends on the statistical test because the MLP
variance is large (Welch $p=0.055$, Mann-Whitney $p=0.026$, Cohen's
$d=-0.98$).  Across the same-scale benchmark as a whole, however, no setting
shows a robust MLP advantage over the linear baseline.
...
transfer signature.  Because this transfer table is reduced-budget and
exploratory, we treat it primarily as a methodological clarification of how
protocol-sensitive R2 is, not as evidence on the same footing as the main
10-seed same-scale benchmark.  In particular, this exploratory whole-lattice
result should not be read as positive RG-like evidence; it more likely reflects
the operational ambiguity of the current transfer protocol.
...
\medskip\noindent
\textbf{Reproducibility.}  The main same-scale benchmark, exploratory
whole-lattice transfer, and mixing diagnostics are all sourced from separate
output directories and are not numerically mixed.  Legacy pilot ablations are
kept separate for appendix-only sanity checks.

\medskip\noindent
\textbf{What a convincing positive result would require.}  A persuasive
positive benchmark for RG-like transfer should satisfy at least four conditions:
matched training budget across same-scale and transfer experiments, a true
multi-scale chain (e.g.\ $4\to8\to16$ together with direct $4\to16$ and
$8\to16$ comparisons), autocorrelation-aware sampling with reported effective
sample sizes, and a transfer regime in which competitive linear baselines
actually fail.
